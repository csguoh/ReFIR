...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        for downsample_block in self.down_blocks: # NOTE Diffusion Encoder 这块和Controlnet使用的moduel都是一样的
        ...
        if is_controlnet: # 合并controlNet和Encoder的结果
            ...
            down_block_res_samples = new_down_block_res_samples # 12 list 最终加到Decoder的控制
        ...
        if self.mid_block is not None:
            ...
            # TODO 计算相似度
            # msk1 = get_sim_mask(sample[0:1], sample[1:2])
            # msk2 = get_sim_mask(sample[2:3], sample[3:4])
            # msk = torch.cat([msk1,msk2]) # [2,1,H,W]
            # global register_mask
            # register_mask = msk
            # msk= msk.reshape(2,-1,1) # [B,L,C]
            # =======================================
        ...
        global register_mask # TODO 每个阶段单独计算一次mask
        # 5. up 
        for i, upsample_block in enumerate(self.up_blocks): # NOTE diffusion Decoder 称为up_blocks, Encoder则称为down_blocks
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                ...
                sample = upsample_block(
                    hidden_states=sample,
                    temb=emb,
                    use_init_align = use_init_align,
                    res_hidden_states_tuple=res_samples, # feat form the controlNet
                    encoder_hidden_states=encoder_hidden_states, # text
                    cross_attention_kwargs=cross_attention_kwargs,
                    upsample_size=upsample_size,
                    attention_mask=attention_mask,
                    encoder_attention_mask=encoder_attention_mask,
                    image_encoder_hidden_states=image_encoder_hidden_states, # sam feat 
                    # register_mask = msk,
                )
            else:
                sample = upsample_block(
...
def get_sim_mask(X,Y):
    # X,Y [B,C,H,W]
    B,C,H,W = X.shape
    X_flat = X.reshape(B, C, H*W).permute(0, 2, 1)  # Size: [B, HW, C]
