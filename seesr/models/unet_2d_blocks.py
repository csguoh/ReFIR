...
def robust_scale(target, source):
    """
    ...
    """
    B,C,H,W = target.size() # [head,C,H,W]

    target = target.view(B,C,-1).permute(0,2,1).contiguous()
    source = source.view(B,C,-1).permute(0,2,1).contiguous()
    ...
    out= target_scaled.view_as(target).to(dtype = target.dtype)
        ...
        if self.use_image_cross_attention:
            ...
            for i, (resnet, attn, image_attn) in enumerate(blocks):
                if self.training and self.gradient_checkpointing:
                    ...
                    if use_init_align: # Noise Alignment
                        hidden_states[0] = adain_latent(hidden_states[0:1],hidden_states[1:2])
                        hidden_states[2] = adain_latent(hidden_states[2:3],hidden_states[3:4])
                    ...
                    hidden_states = attn( # 这个里面同时包含了self和cross
                        hidden_states, # 图片
                        encoder_hidden_states=encoder_hidden_states, # 文本提示
                        cross_attention_kwargs=cross_attention_kwargs,
                        attention_mask=attention_mask,
                        encoder_attention_mask=encoder_attention_mask,
                        return_dict=False,
                    ...
                    hidden_states = image_attn(
                        hidden_states,
                        encoder_hidden_states=image_encoder_hidden_states, # [2,145,512] 图像的交叉注意力
                        cross_attention_kwargs=cross_attention_kwargs,
                        attention_mask=attention_mask,
                        encoder_attention_mask=encoder_attention_mask,
                        return_dict=False,
    ...
    def forward(
        ...
        if self.use_image_cross_attention:
            for resnet, attn, image_attn in zip(self.resnets, self.attentions, self.image_attentions):
                # pop res hidden states
                res_hidden_states = res_hidden_states_tuple[-1] # 来自encoder和contrl的条件，这里使用了拼接来处理
                res_hidden_states_tuple = res_hidden_states_tuple[:-1]
                hidden_states = torch.cat([hidden_states, res_hidden_states], dim=1)
                ...
                if self.training and self.gradient_checkpointing:
                    ...
                    decoder_h, decoder_w = hidden_states.shape[-2:]


                    hidden_states = attn( # 同时包含了self 以及 cross attn 的
                        hidden_states, 
                        encoder_hidden_states=encoder_hidden_states, # 文本的条件
                        cross_attention_kwargs=cross_attention_kwargs,
                        attention_mask= attention_mask,
                        encoder_attention_mask=encoder_attention_mask,
                        return_dict=False,
                    ...
                    hidden_states = image_attn(
                        hidden_states,
                        encoder_hidden_states=image_encoder_hidden_states, # 图像的条件
                        cross_attention_kwargs=cross_attention_kwargs,
                        attention_mask=attention_mask,
                        encoder_attention_mask=encoder_attention_mask,
                        return_dict=False,
