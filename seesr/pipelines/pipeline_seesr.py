...
def adain_latent(feat, cond_feat, timestep=0, eps=1e-5):
    # eps is a small value added to the variance to avoid divide-by-zero.
    B,C,H,W = feat.size() # [head,C,H,W]

    feat = feat.view(B,C,-1).permute(0,2,1).contiguous()
    cond_feat = cond_feat.view(B,C,-1).permute(0,2,1).contiguous()
    ...
    weight = cosine_schedule(1000-timestep, 1000-0)
    target_std = (weight*feat_std + (1-weight)*cond_feat_std)
    target_mean= (weight*feat_mean + (1-weight)*cond_feat_mean)
...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def _encode_prompt(
        ...
        prompt_embeds = prompt_embeds.to(dtype=self.text_encoder.dtype, device=device) #[2,77,C]
        ...
        if do_classifier_free_guidance:
            ...
            prompt_embeds = torch.cat([negative_prompt_embeds, prompt_embeds]) # NOTE 这里竟然是把neg的pmt放在的前面
    ...
    def prepare_image(
        self,
        image, # 支持-- cat的tensor(这个比较适合我们)，list形式的tensor，list形式的PIL(前提是两个PIL的HW维度必须相同)，单独一个PIL
        width,
        height,
        batch_size,
        num_images_per_prompt,
    ...
    def __call__(
        ...
        ) # [4(neg(inp+ref) + pos), 77, C], [2, 145, 512]

        # 4. Prepare image
        image = self.prepare_image(
            image=image, # PIL or pre-procesed tensor [2,C,h,w] 
            width=width, 
            height=height,
        ...
        # TODO ========== 这个随机噪声是否需要是两个图像都一样呢？================
        # 6. Prepare the start point
        if start_point == 'noise':
            latents = latents
        elif start_point == 'lr': # LRE Strategy
            latents_condition_image = self.vae.encode(image*2-1).latent_dist.sample() # [4,4,H,W]
            ...
            latents = self.scheduler.add_noise(latents_condition_image[:latents_condition_image.shape[0]//2, ...], latents, start_steps_tensor) # TODO 别人这样做本质上是利用了广播机制，因为对于单张图像来说的话用第一个和都用是一样的，就我们这个来说我们应该使用inp-ref的未复制前版本，就是使用前两个，加的噪声应该是相同的。
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            _, _, h, w = latents.size() # [2,4,75,75]
            ...
            for i, t in enumerate(timesteps):
                ...
                if t > start_steps:
                    ...
                    continue
                #  这个地方挺有意思的，在prepare的时候已经复制了，但是这里还要复制，一开始的复制是多余的！！！
                # expand the latents if we are doing classifier free guidance
                latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents # [4,4,h,w]
                ...
                if True: #h*w<=tile_size*tile_size: # tiled latent input
                    down_block_res_samples, mid_block_res_sample = [None]*10, None
                    down_block_res_samples, mid_block_res_sample = self.controlnet( # 注意这里的ctrolNet的调用可不止这一个地方！！！
                    ...
                    noise_pred = self.unet( # TODO 劫持这个unet
                        latent_model_input,
                        t,
                        encoder_hidden_states=prompt_embeds, # text embedding
                        cross_attention_kwargs=cross_attention_kwargs, 
                        down_block_additional_residuals=down_block_res_samples, # 来自controlNet的控制
                        mid_block_additional_residual=mid_block_res_sample, # 来自ControlNet中间层的控制
                        return_dict=False,
                        image_encoder_hidden_states = ram_encoder_hidden_states,
                    )[0] # [2,4,75,75]
                else:
                    ...
                    for row in range(grid_rows):
                        ...
                        for col in range(grid_cols):
                            ...
                            if len(input_list) == batch_size or col == grid_cols-1:
                                input_list_t = torch.cat(input_list, dim=0) # [2,4,80,80]
                                cond_list_t = torch.cat(cond_list, dim=0) # [2,4,80,80]
                                img_list_t = torch.cat(img_list, dim=0) # [4,3,640,640]
                                #print(input_list_t.shape, cond_list_t.shape, img_list_t.shape, fg_mask_list_t.shape)

                                down_block_res_samples, mid_block_res_sample = self.controlnet(
                                    cond_list_t,
                                    t,
                                    encoder_hidden_states=controlnet_prompt_embeds, # [4,77,1024]
                ...
                # TODO ==================================时间角度的对齐==============
                # if t>0:
                #     latents[0] = adain_latent(latents[0:1], latents[1:2],t)
        ...
        has_nsfw_concept = None
        if not output_type == "latent": # VAE的Decoder
        ...
        # TODO 这里把tensor变成了PIL-List！！！  而且还是导的包
