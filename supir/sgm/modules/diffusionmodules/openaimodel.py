...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(
        ...
        use_crossframe_attention_in_spatial_layers=False,n_times_crossframe_attn_in_self=False,
    ):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context,n_times_crossframe_attn_in_self=n_times_crossframe_attn_in_self)
            else:
                x = layer(x)
        return x
