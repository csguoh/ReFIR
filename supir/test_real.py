...
for img_pth in sorted(os.listdir(args.img_dir)):
    ...
    if 'CUFED5' in args.img_dir: 
        #ref_img = Image.open(os.path.join(args.img_dir, img_pth)) #Image.open(os.path.join("/home/tiger/gh/dataset/CUFED5/Real_Deg/HR",img_name+'_0_HR.png'))
        #rand_name = random.choice([p for p in sorted(os.listdir(os.path.join(args.img_dir.replace('Real_Deg/LR', 'ref')))) if '_1.png' in p])
    ...
    for _i, sample in enumerate(samples):
        Tensor2PIL(sample[:, :LQ_h_resize, :LQ_w_resize], scale*h_LQ, scale*w_LQ).save(f'{args.save_dir}/{img_name}_0_LR.png')
        break
